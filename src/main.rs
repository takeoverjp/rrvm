...
mod tests {
    ...
    fn test_lw() {
        ...
        let mem = Memory::new(&mut vec, &elf);
        // lw r2, 3(r1)
        let inst: u32 = inst_lw(2, 3, 1);
        reg.x[1] = 0x1;

        handle_load(&mem, &mut reg, inst);

        assert_eq!(0x07060504, reg.x[2]);
    }

    #[test]
    fn test_lw_negative_value() {
        let mut reg = RegisterFile::new();
    ...
    fn test_lw_negative_offset() {
        ...
        let mem = Memory::new(&mut vec, &elf);
        // lw r2, -3(r1)
        let inst: u32 = inst_lw(2, -3i16 as u16, 1);
        reg.x[1] = 7;
        handle_load(&mem, &mut reg, inst);
        assert_eq!(0x07060504, reg.x[2]);
    }
    ...
    fn test_ori_pos() {
        let mut reg = RegisterFile::new();
        let inst: u32 = inst_ori(2, 1, 0b0010101_01010);
    ...
    fn test_ori_neg() {
        let mut reg = RegisterFile::new();
        let inst: u32 = inst_ori(2, 1, 0b1010101_01010);
        reg.x[1] = 0b010111000000;
        handle_op_imm(&mut reg, inst);
    ...
    fn test_andi() {
        let mut reg = RegisterFile::new();
        let inst: u32 = inst_andi(2, 1, 0b1010101_01010);
